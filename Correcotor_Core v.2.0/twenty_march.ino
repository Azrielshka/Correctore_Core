...
#define BOT_TOKEN "8174224424:AAEwywZOjroal-4ocxz8vRUgLz8iOG1m4wA"
...
void loop() {
  ...
  //---верхний датчик---------------  
 if (millis() - tmr4 >= 2000) {      // таймер на 2 с (на всякий случай)
...
void initialize_mpu1 () {
  ...
  if (devStatus == 0) {
    ...
    bot.sendMessage(chat_id, "good mpu1", "");
    ...
    Serial.println(F(")"));
    bot.sendMessage(chat_id, "not good mpu1", "");
  }
}
...
void initialize_mpu2 () {
  ...
  if (devStatus == 0) {
    ...
    bot.sendMessage(chat_id, "good mpu2", "");
...
void kalib_mpu1 () {
  ...
  Serial.print(gy); Serial.print(" ");
  Serial.println(gz);
  String message = "градусы 1 датчик:\n"
                   "ax: " + String(ax) + "\n"
                   "ay: " + String(ay) + "\n"
                   "az: " + String(az) + "\n"
                   "gx: " + String(gx) + "\n"
...
void kalib_mpu2 () {
  ...
  Serial.print(gy); Serial.print(" ");
  Serial.println(gz);
  String message = "градусы 2 датчик:\n"
                   "ax: " + String(ax) + "\n"
                   "ay: " + String(ay) + "\n"
                   "az: " + String(az) + "\n"
                   "gx: " + String(gx) + "\n"
...
void handleNewMessages(int numNewMessages)
{
  ...
  for (int i = 0; i < numNewMessages; i++)
  {
    ...
    if ((text == "/calibration_of_sensors") || (text == "calibration_of_sensors"))
        {
            calibrateSensors();
        } 
    if ((text == "/calibration_for_humans") || (text == "calibration_for_humans")) {
            calibrateForHumans();
           
        }
         
      
    if ((text == "/initialize") || (text == "initialize"))
        {
            initialize_mpu1();
            initialize_mpu2();
     ...
     if ((text == "/count") || (text == "count"))
        {
           bot.sendMessage(chat_id, String(count1), "");
   ...
   if ((text == "/start") || (text == "start") || (text == "/help") || (text == "help"))
    {
      ...
      welcome += "Я принимаю следующие команды:\n";
       welcome += "\n";
      welcome += "/calibration_for_humans или /calibration_of_sensors - включить/выключить светодиод.\n";
       welcome += "\n";
      welcome += "/initialize - включить/выключить светодиод.\n";
      welcome += "\n";
      welcome += "Команды можно писать без слэша слева и буквами любого регистра.\n";
      welcome += "Инструкция:\n";
      welcome += "\n";
      welcome += "Сначала нужно нажать на команду /initialize. Если пришло сообщение good, то вы должны нажать на команду /calibration_of_sensors, а если вы увидели сообщение not good, то нужно начать калибровку занаво.\n";
      welcome += "\n";
      welcome += "После нажмите /calibration_for_humans\n";
      welcome += "\n";
