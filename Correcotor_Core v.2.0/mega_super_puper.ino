...
void calibrateSensors() {
 //----------Калибровка датчиков----------------------
  ...
  if (knopka){
    ...
    delay(300);
    break;
  bot.sendMessage(chat_id, "колибровка датчиков завершена", ""); 
}}}
...
//----------Калибровка под человека----------------------
  ...
  while (1){
    ...
    bot.sendMessage(chat_id, "колибровка датчиков под человека завершена", "");   
}
}
void handleNewMessages(int numNewMessages)
{
  ...
  for (int i = 0; i < numNewMessages; i++)
  {
    ...
    if ((text == "/calibration of sensors") || (text == "calibration of sensors"))
        {
            calibrateSensors();
        } else if ((text == "/calibration for humans") || (text == "calibration for humans")) {
            calibrateForHumans();
   ...
   if ((text == "/start") || (text == "start") || (text == "/help") || (text == "help"))
    {
      ...
      bot.sendMessage(chat_id, "Я принимаю следующие команды:", "");
      bot.sendMessage(chat_id, "/calibration for humans или /calibration of sensors - включить/выключить светодиод", "");
